...
def build_service(*, symbol: str = "XAUUSD", shadow: bool = True,
    ...
    #
    # This was the last unwired link on the macro path: macro_context could
    # build a block and MarketBrief could carry one, but nothing ever
    # CONSTRUCTED a provider, so the analyst would have read UNMEASURED
    # forever while every individual part looked correctly wired.
    macro_fn = None
    if enable_macro:
        def macro_fn():

...
def _trade(obs, mfe=0.0, mae=0.0):
    return {"kind": "TRADE_CLOSED", "observations": obs, "mfe_r": mfe, "mae_r": mae}
...
def test_pre_fix_trades_no_longer_hold_the_check_broken_forever():
    ...
    rows = [_trade(0), _trade(0)] + [_trade(120, 1.4, -0.6) for _ in range(5)]
...
def test_a_REGRESSION_after_the_fix_still_fails():
    ...
    rows = [_trade(0), _trade(120, 1.4, -0.6), _trade(0), _trade(90, 1.0, -0.3)]
    f = check_excursion_survives(rows)
    assert not f.ok
